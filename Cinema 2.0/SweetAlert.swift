...
open class SweetAlert: UIViewController {
    ...
    var heightViewChangePassword:CGFloat = 0
    ...
    fileprivate func resizeAndRelayout() {
        ...
        contentView.frame = CGRect(x: (mainScreenBounds.size.width - kContentWidth) / 2.0, y: (mainScreenBounds.size.height - y) / 2.0 - heightViewChangePassword/3, width: kContentWidth, height: y)
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
        _ = showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: UIColor.iconSelected)
        userAction = action
        return self
    }
...
extension SweetAlert{
    
    func showChangePassword(user:User){
        ...
        let textFieldNewPassWorld:UITextField = {
            let tf = UITextField()
            tf.font = UIFont(name: AppConstants.FontName.regular, size: 16)
        ...
        contentView.addViewWithContrains(VSFormat: "H:|-30-[v0]-60-|", views: textFieldPassWorld)
        contentView.addViewWithContrains(VSFormat: "H:|-30-[v0]-60-|", views: textFieldNewPassWorld)
        contentView.addViewWithContrains(VSFormat: "H:|-30-[v0]-60-|", views: textFieldCFPassWorld)
        contentView.addViewWithContrains(VSFormat: "V:[v0(40)]-3-[v1(40)]-3-[v2(40)]-80-|", views: textFieldPassWorld, textFieldNewPassWorld, textFieldCFPassWorld)
        heightViewChangePassword = 135
        
        self.titleLabel.text = "Đổi mật khẩu"
        self.subTitleTextView.text = ""
        buttons = []
        
        let button: UIButton = UIButton(type: UIButtonType.custom)
        button.setTitle("Cập nhật", for: UIControlState())
        button.backgroundColor = .iconSelected
        button.addTarget(self, action: #selector(SweetAlert.pressed(_:)), for: UIControlEvents.touchUpInside)
        button.isUserInteractionEnabled = true
        button.tag = 0
        buttons.append(button)
        ...
        otherButton.setTitle("Huỷ", for: UIControlState())
        otherButton.backgroundColor = .orange
        otherButton.addTarget(self, action: #selector(SweetAlert.pressed(_:)), for: UIControlEvents.touchUpInside)
        otherButton.tag = 1
